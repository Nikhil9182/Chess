...
namespace Chess.Board.Core
{
    public class MoveGenerator
    {
        ...
        public static void GenerateMoves()
        {
            ...
            {
                int piece = BoardHandler.Square[startSquare];

                if (Piece.IsColor(piece, BoardHandler.ColorToMove))
                {
        ...
        public static List<Move> GenerateSlidingPieceMoves(int piece, int startSquare)
        {
            ...
            for (int directionIndex = startDirIndex; directionIndex < endDirIndex; directionIndex++)
            {
                for (int n = 0; n < NumSquaresToEdge[startSquare][directionIndex]; n++)
                {
                    ...
                    int pieceOnTargetSquare = BoardHandler.Square[targetSquare];
        ...
        public static List<Move> GeneratePawnMoves(int piece, int startSquare)
        {
            ...
            if (oneStep >= 0 && oneStep < 64 && BoardHandler.Square[oneStep] == Piece.None)
            {
                ...
                if ((Piece.IsColor(piece, Piece.White) && startRank == 1) || (Piece.IsColor(piece, Piece.Black) && startRank == 6))
                {
                    ...
                    if (BoardHandler.Square[twoStep] == Piece.None)
                    {
            ...
            if (leftSquare >= 0 && leftSquare < 64)
            {
                ...
                if (Mathf.Abs(startFile - targetFile) == 1 && Piece.IsColor(BoardHandler.Square[leftSquare], opponentColor))
                {
                    if (targetRank == 0 || targetRank == 7) // Promotion condition
                    {
                        // Add promotions to all piece types
            ...
            if (rightSquare >= 0 && rightSquare < 64)
            {
                ...
                if (Mathf.Abs(startFile - targetFile) == 1 && Piece.IsColor(BoardHandler.Square[rightSquare], opponentColor))
                {
                    if (targetRank == 0 || targetRank == 7) // Promotion condition
                    {
                        // Add promotions to all piece types
            ...
            // --- En Passant Capture ---
            if (BoardHandler.EnPassantSquare != -1) // You need to store this in Board after double push
            {
                // Left en passant
                if (startFile > 0 && BoardHandler.EnPassantSquare == startSquare + captureLeft)
                {
                    generatedMoves.Add(new Move(startSquare, BoardHandler.EnPassantSquare, Move.EnPassantCapture));
                }
                // Right en passant
                if (startFile < 7 && BoardHandler.EnPassantSquare == startSquare + captureRight)
                {
                    generatedMoves.Add(new Move(startSquare, BoardHandler.EnPassantSquare, Move.EnPassantCapture));
                }
            }
        ...
        public static List<Move> GenerateKingMoves(int piece, int startSquare)
        {
            ...
            if (!Piece.HasMoved(piece))
            {
                ...
                int rookPieceKingside = BoardHandler.Square[rookSquareKingside];
                ...
                {
                    if (BoardHandler.Square[startSquare + 1] == Piece.None &&
                        BoardHandler.Square[startSquare + 2] == Piece.None &&
                ...
                int rookPieceQueenside = BoardHandler.Square[rookSquareQueenside];
                if (Piece.IsType(rookPieceQueenside, Piece.Rook) && !Piece.HasMoved(rookPieceQueenside) && Piece.IsColor(rookPieceKingside, friendlyColor))
                {
                    if (BoardHandler.Square[startSquare - 1] == Piece.None &&
                        BoardHandler.Square[startSquare - 2] == Piece.None &&
                        BoardHandler.Square[startSquare - 3] == Piece.None &&
                        !AttackSquares.Contains(startSquare) &&
                        !AttackSquares.Contains(startSquare - 1) &&
                        !AttackSquares.Contains(startSquare - 2))
                    {
